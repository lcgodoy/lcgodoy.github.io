...
- The model of choice [depends]{.important} heavily on the spatial
...
## Preliminaries: Isotropic GPs

- [Gaussian process]{.important2}: A stochastic process such that all its
  finite-dimensional marginal distributions are Gaussian.
  
- Completely characterized (probabilistically) by two functions:
  - The [mean function]{.important}: $m(\mathbf{s}_1) = \mathbb{E}[Z(\mathbf{s}_1)]$
  - The [covariance function]{.important}: $$\mathrm{C}(Z(\mathbf{s}_1),
    Z(\mathbf{s}_2)) = \mathrm{Cov}(Z(\mathbf{s}_1), Z(\mathbf{s}_2)).$$

- A GP is [isotropic]{.important} if $\mathrm{Cov}(Z(\mathbf{s}_1),
  Z(\mathbf{s}_2)) = \mathrm{C(d(\mathbf{s}_1, \mathbf{s}_2))}$, where $d$ is a
  distance function.

::: aside 
> $\{ Z(\mathbf{s}) \, : \, \mathbf{s} \in D \}$ is a stochastic process and
> $\mathbf{s}_1, \mathbf{s}_2 \in D$.
:::


## Preliminaries: Sets and distances
...
- [Study region]{.important2}: In spatial statistics, $D$ is tipically a
  [non-empty, closed and bounded]{.important} subset of $\mathbb{R}^2$.
...
## The Hausdorff--Gaussian Process

* [Index set]{.important2}: $\mathcal{B}(D)$ represents the [non-empty, closed
  and bounded]{.important} subsets of $D \subset \mathbb{R}^2$.
...
- [Cov. function]{.important2}: $\mathrm{Cov}(Z(\mathbf{s}_1),
...
- Useful special cases:
    ...
    - [Homoscedastic]{.important2}: $w(\mathbf{s}) = 0$ ($\therefore \sigma =
      \exp \{ \alpha_0 \}$)
...
:::{.incremental}
- [Parametrization]{.important2}: $\rho = {\log(10)}^{1 / \nu} \phi$.
...
# Illustration with real data {.section_slide}

## Spatial GLMM {visibility="uncounted"}
...
## Bayesian inference \& model assessment {visibility="uncounted"}
...
## Bayesian modeling recap {visibility="uncounted"}
...
# Research breadth {.section_slide visibility="uncounted"}

## Work in progress (Theory)

- Radially [positive-definite functions]{.important} of the [Hausdorff
  distance]{.important} on non-Euclidean spaces(with Marcos Prates, Fernando
  Quintana and Jun Yan)

- HGP, [big data]{.important} and [INLA]{.important} (with Marcos Prates and Jun
  Yan)

- Nearest-neighbor mixture processes and the Hausdorff distance (with Bruno
  Sansó)

- [Neural-networks]{.important} and spatiotemporal [data fusion]{.important}
  (with Lucas Michelin, Marcos Prates, and Abhi Datta)

- Multiplier bootstrap and [fast inference]{.important} for
  [auto-logistic]{.important} models (with Yuan Huang and Jun Yan)

## Work in progress (Application)

- Review paper on [sports analytics]{.important} (with Greg J. Matthews and Jun
  Yan)

- Combining [Bayesian]{.important} hierarchical models \& [population
  dynamics]{.important} for species distribution models (with Malin Pinsky)

- `R` package to estimate [risks]{.important} in [consumer automobile
  loans]{.important} (with Jackson Lautier)
  
- [Non-stationary geostatistical]{.important} models for stock assessment in
  [fisheries]{.important} (with James Thorson)

- [Non-linear mixed effect models]{.important} to analyze plants
  [heat-tolerance]{.important} changes in face of [climate change]{.important}
  (with Georgia Hernández)

## Other contributions

- Organization of weekly meetings and extracurricular projects for students.

- Co-advised a Master's student in Brazil [@michelin2025fast]

- Many papers as a result of my work as statistical consultant. _Highlights_:
  @andreyeva2024federal and @sieger2025effect


<!-- ## Future work -->
<!-- - Flexible separable spatiotemporal covariance functions based on the HGP -->
<!-- - Gaussian processes on polygonal windows - current solutions rely on stochastic -->
<!--   partial differential equations (SPDE) -->
<!-- - Spectral analysis on the topology induced by the Hausdorff distance -->
<!-- - Amortized inference \& geostatistics -->
<!-- - Random set theory to quantify species' range shifts -->
