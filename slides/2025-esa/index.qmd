...
title-slide-attributes:
  data-background-image: img/qr-code.png
...
:::{.incremental}
* $Y_{t, i}$ is the [observed density]{.important} of individuals (all ages) at
  time $t$ and site $i$, and it follows a [zero-augmented probability
...
## Model assumptions! {visibility="uncounted"}
...
:::{.incremental}
* The [recruitment]{.important} density is driven by the environmental
  conditions in the current year: $$ \log(\lambda_{1, t, i}) =
  \mathbf{x}^{(r)}_{t, i} \mathbf{\beta}_{r} + z^{(r)}_{t, i} $$

* The [density of individuals]{.important} who [survive from the previous
  year]{.important} and their respective [estimated survival rate]{.important}:
  $$ \lambda_{a, t, i} = s_{a - t, t - 1, i} \cdot \lambda_{a - t, t - 1, i}, $$
  where $$\log(s_{a, t, i}) = \mathbf{x}^{(s)}_{t, i}
  \mathbf{\beta}_{s} + z^{(s)}_{t, i} - f_{a, t}$$
:::
...
![](img/sf.png){.absolute bottom=10 right=85 width="120" height="120"}

::: aside
Figure source: [ASMFC](https://asmfc.org/species/summer-flounder/)
:::
...
* The `drmr` substantially [lowers the barrier]{.important} for
  [ecologists]{.important} to use the [DRM in their applications]{.important}.

* The code is [easy to use]{.important} and takes advantage of what has been
  developed for `Stan`: visualization, diagnostic tools, and estimation.

* `drmr` allows for [empirically testing]{.important} which
  [processes]{.important} are more [important to predict]{.important} the
  distribution of a [species]{.important}.
...
* Include population dynamic models that [explicitly relate adult density to
  recruitment]{.important} (e.g., Ricker, Belverton-Holt)

* [GAM]{.important}-like non-parametric relationships between [processes and the
  environment]{.important}

* Support for [length-composition data]{.important}

* More [realistic movement/dispersal]{.important} routines
