...
- [TB in Context]{.important2}: The United Nations' [Millennium Declaration in
  2000]{.important} outlined eight development goals to be achieved by 2015,
  including combating Tuberculosis (TB).

- [Mortality]{.important2}: TB remains a major global health threat, [second in
  infectious disease mortality]{.important} only to COVID-19.
  
- [Brazilian TB Trends]{.important2}: Brazil's national TB incidence declined
  only slightly from 2006--2015. 
...
## Previous Analyses \& Risk Factors

- [Spatial Dependence]{.important2}: Studies demonstrate [strong spatial
  dependence]{.important} of TB infections in Brazil, both nationwide and in
  specific cities. 
  
- [Temporal dependence]{.important2} has been largely overlooked in the
  literature.
...
$$\begin{aligned}
& Y_t(\mathbf{s}_i) \mid \mathbf{x}_{it}, Z(\mathbf{s}_i, t) \overset{{\rm ind}}{\sim}
  f(\cdot \mid \mu_{it}, \boldsymbol{\gamma}) \\
& g(\mu_{it}) = \alpha + \mathbf{x}_{it} \boldsymbol{\beta} +
  Z(\mathbf{s}_i, t).
\end{aligned}$$

- [Probability distribution]{.important2}: $f(\cdot)$

- [Conditional mean]{.important2}: $\mu_{it} = \E[Y_t(\mathbf{s}_i) \mid
  \mathbf{x}_i, Z(\mathbf{s}_i, t)]$
...
- [Joint density]{.important2}: $p(\mathbf{y} \mid \mathbf{z},
  \boldsymbol{\theta}) = \prod_{i = 1}^n f(y_t(\mathbf{s}_i) \mid \mu_{it},
  \boldsymbol{\gamma})$
...
- We assume $Z(\mathbf{s}, t)$ is a separable zero-mean GP with a covariance
  function that is the product between the HGP covariance function and the
  $\mathrm{AR}(1)$ covariance function.


- [Spatial dependence]{.important2}: $\rho \sim \mathrm{Exp}(a_\rho)$, where
...
- [Super naive]{.important2}: $vec(\mathbf{Z}) \sim \mathcal{N}(\mathbf{0},
  \sigma^2 \mathrm{R}_s \otimes \mathrm{R}_t)$ [requires $\mathcal{O}(N^3 T^3)$
  _flops_ (and storage)]{.important}.
  

- [Naive]{.important2}: With linear algebra, we can reduce the computational
  [complexity (and storage) to $\approx \mathcal{O}(N^3 + T^3)$]{.important}


- [Astute]{.important2}: More linear algebra can be used to evaluate a
...
# Modeling TB {.section_slide visibility="uncounted"}
