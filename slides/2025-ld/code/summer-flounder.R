...
set.seed(2025)
sel_patches <- sample(seq_len(max(fitted_sdm$patch)), 3) |>
  sort()

forecast_fig <- bind_rows(fitted_sdm, for_sdm) |>
  mutate(model = "SDM") |>
  bind_rows(
...
saveRDS(forecast_fig,
        file = "slides/2025-esa/data/for_fig.rds")
...
saveRDS(rec_summary, file = "slides/2025-esa/data/rec.rds")
saveRDS(surv_summary, file = "slides/2025-esa/data/surv.rds")
